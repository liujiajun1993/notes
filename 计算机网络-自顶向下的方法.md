...
### 4.2 网络层协议
因特网协议第四版(IPv4)负责网络层的分组、转发以及传递。因特网控制报文协议第四版(ICMPv4)帮助IPv4处理一些网络层中传递中可能发生的错误。因特网组管理协议(IGMP)用于帮助IPv4多播。地址解析协议(ARP)用来将网络和数据链路层联合起来。
...
<span style="color:#f00">动态主机配置协议DHCP</span>
...
因特网控制报文协议（ICMP）配合IP协议使用，在进入较低层前，报文被封装到IP数据报中，并将IP数据报的响应协议字段值设为1，表明该IP负载时一个ICMP报文。
...
最小代价路由
最小代价树，可以根据动态规划思想建立，Dijkastra算法
