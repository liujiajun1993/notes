...
3. TCP/IP协议簇    
   ...
   3. 网络层，在源计算机和目标计算机之间创建一个连接。IP/DHCP/ARP（地址解析协议）/ICMP（因特网控制报文协议）/IGMP（因特网组管理协议）
   4. 数据链路层，将数据包封装在frame（帧）中
   5. 物理层。携带frame中的bit穿过链路
  
    ![协议分层](http://mmbiz.qpic.cn/mmbiz_png/CP9AlgoibiagUCusWVjTfLkA0BucORMejrX9sYLPewYtibkw1HhI9XHoWFoO5yh9QuahKdsPNyDHR8jiba9OLah9KA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1)

4. 封装和解封装
  应用层message -> 传输层segment(TCP)/user datagram(UDP) -> 网络层(datagram, 添加自己的头部) -> 数据链路层(frame, 添加自己的头部) -> 物理层    
  ![封装过程](http://mmbiz.qpic.cn/mmbiz_png/CP9AlgoibiagUCusWVjTfLkA0BucORMejruzrkN8PL1dGtQ3KsnasXeAwaqdVTichXgvKzXt8Snial5D2b5hhD7Z1Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1)
...
### 2.1 客户-服务器模式
   ...
   **HTTP安全**：HTTPS提供保密性、客户和服务器鉴别，以及数据完整性保证。
   ...
   <em>SSH</em>    
   ![SSH组件](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C2-7.png)    
   SSH传输层协议建立在TCP协议上，建立安全信道，保证保密性、数据完整性、服务认证和报文压缩。
   SSH认证协议让服务器认证客户。
   SSH连接协议用于通信逻辑。

   <em>DNS</em>   
   ![DNS名字服务器](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C2-8.png)
...
套接字结构的<mark>五个字段</mark>
   - 族（协议簇，通常是PF\_INET/PF\_INET6）
   - 类型（SOCK\_STREAM用于TCP，SOCK\_DGRAM用于UDP，SOCK\_SEQPACKET用于，SOCK\_RAW用于ISP服务）
...
TCP使用<span>字节流</span>传送数据，但是IP使用<span>分组</span>方式而非字节流。因此在传输层，TCP将多个字节组合在一起成为一个分组，称为段（segment），给每个段添加头部，并将该段传递给IP层。
...
1. <span>连接建立</span>
  ...
  ![三次握手过程2](http://mmbiz.qpic.cn/mmbiz_png/CP9AlgoibiagUCusWVjTfLkA0BucORMejr7HHChp8nib2ugE9A1mBpiaq8YF7M3mmdGqwnhCSrDL5O2siaNOtAmYJTQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1) 
...
<em>MAC子层</em>
![MAC层](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-1.png)
...
- 确认。正式确认和超时定时器。
![CSMA/CA的流程图](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-2.png)
...
<span>握手</span>
![CSMA/CA和NAV](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-3.png)
...
<em>帧格式</em>
![帧格式](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-4.png)
帧类型： 管理帧、控制帧、数据帧
...
<mark>微微网络</mark>
微微网络可以包含多达8个站点，其中一个主站点，其他为从站点。额外的从站点可以处于休眠状态，但是如果需要激活额外的从站点，则有一个活动从站点需要进入休眠。
<mark>散射网络</mark>
微微网络可以组成散射网络。
微微网络中的从站，可以是另一个微微网络的主站。
![散射网络](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-5.png)

<mark>蓝牙层</mark>
![蓝牙层](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-6.png)
<span>L2CAP</span>（逻辑连接控制和自适应协议）层大概相当于局域网中的LLC子层。有以下几个作用：多路复用、分段和重组、服务质量和组管理。
<span>基带层</span>，大致相当于局域网中的MAC层。访问方法为TDMA，时分多址。
蓝牙使用<span>时分双工TDMA</span>，半双工通信方式。主站使用偶数时隙发送，从设备接受；从设备们在奇数时隙内依次发送，主设备接收。
链路方式
- SCO，避免延迟比完整性更重要，分组被破坏不重传
- ACL，完整性比延迟更重要，需要重传。
<span>无线电层</span>，大致相当于物理层。蓝牙设备内置短程无线电发射机，2.4GHz下的1Mbps。范围是10m。
蓝牙使用跳频扩频方法来避免干扰，每秒调频1600次，每个频率维持625μs。

<em>WiMAX</em>
全球互通微波存取，目标是提供“最后一公里”的宽带无线接入，可替代调制解调器或电话DSL。
类比于WiFi，但覆盖范围更广，安全性、可靠性、吞吐量更高。
体系结构：
- 基站。无线电广播设备和天线。
- 用户站点
- 便携设备

WiFi在MAC层采用竞争访问，导致远离访问点的站点经常被更近的站点中断；WiMAX采用调用策略。

### 6.2 其他无线网络
<em>频道化</em>
- <span>FDMA</span>
- <span>TDMA</span>
- <span>CDMA</span>站点使用不同的编码。

<em>蜂窝电话</em>
服务商定位并追踪主叫方，为呼叫分配一个频道，当主叫方移动到范围之外，将通道从一个基站切换到另一个基站。
每个蜂窝服务区域成为信元。相邻信元不能使用同一套频率，但是不相邻信元可以复用频率。
<span>呼叫。</span>移动站点扫描频带寻找一个信号强的设置通道，使用该通道向最近站点发送数据（电话号码）。基站向移动交换中心（MSC）发送，MSC向电话局发送。如果被叫空闲，建立连接并返回。
<span>寻呼。</span>MSC向每个信元发送查询信号来搜索被叫移动站点的位置。
<span>漫游。</span>相邻的移动服务商通过漫游合约提供扩展的覆盖范围。

<span>1G</span>使用FDMA
<span>2G</span>使用GSM和CDMA
<span>3G</span>提供数字数据和语音通信技术

<em>卫星网络</em>

### 6.3 移动IP
IP协议的扩展，允许移动设备在任意位置连接至互联网
<em>寻址</em>
- 改变地址：当移动主机去往一个新的网络，让移动站点改变其地址。缺点众多……
- 两个地址：主机有一个归属地址的原始地址和转交地址的临时地址。归属地址永久不变，将主机和所属网络联系起来，该网络是主机永久不变的归属地。转交地址是临时地址，当网络改变时，转交地址就会改变，与外地网络相连

<em>代理</em>
- 归属代理：远程主机向移动主机发送分组时，归属代理表示移动主机，归属代理接受该分组并将其发送至外地代理。
- 外地代理：通常是附属于外地网络的一个路由器。外地代理接受归属代理的数据，并将其发送至移动主机。

<em>三个阶段</em>
![移动IP通信过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-7.png)
- 代理发现
   - 代理通告：路由器使用ICMP路由通告告知网络中它的存在
   - 代理请求：当移动主机移动到新的网络，且没有收到代理通告，则发送代理请求来通知一个代理需要帮助
- 注册
  移动主机移动到外地网络并发现外地代理后，必须注册
  - 移动主机必须在外地代理上注册
  - 移动主机必须在归属主机上注册，通常有外地代理代表移动主机完成
  - 如果移动主机已经过期，需要重新注册
  - 移动主机回到归属地，必须取消登记
- 数据传输
  ![移动IP数据传输过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C6-8.png)
  - 远程主机到归属代理，使用归属地址作为目的地址
  - 归属代理到外地代理
  - 外地代理到移动主机
  - 移动主机到远程主机。使用归属地址作为源地址
...
离散值的信号
比特率：每秒发送的位数，bps
数字信号可以看成由零到无穷大范围的频率组合成的复合模拟信号

<em>传输方式</em>
<mark>基带传输</mark>。通过通道发送数字信号，不转换为模拟信号。需要一个低通通道。不允许同一时刻超过两个站点进行通信。
<mark>宽带传输</mark>。把数字信号转换成模拟信号进行传输。允许我们使用带通通道，该通道的带宽不是从0开始。
![数字信号在带通通道传输的调制过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C7-1.png)
...
<em>速率限制</em>
速率取决于三种因素：有效带宽、信号电平数、通道的质量

<em>性能</em>
带宽：
- 以赫兹衡量的带宽，是符合信号分组包含的频率范围或者通道可以通过的频率范围
- 以每秒位数衡量的带宽
- 两者之间有明显的关系，赫兹衡量的带宽的增长意味着每秒位数衡量的增长

吞吐量：衡量通过网络发送数据的快慢。宽带是链路的潜在衡量值，而吞吐量是实际衡量值。吞吐量永远小于等于带宽。

延迟：延迟 = 传播延迟 + 传输延迟 + 排队延迟 + 处理延迟
<span>带宽延迟乘积定义了能充满链路的位数</span>

抖动：如果数据的不同分组碰到不同的延迟并且应用在接收站使用的数据是时间敏感的。。。。
...
### 7.3 模拟传输
<em>数字到模拟转换</em>
通过改变简单正弦波的某一特性（振幅、频率和相位），可以用来表述数字数据
- 幅移键控（改变振幅，两种振幅表示0或1,；甚至可以分多档0-4等）
- 频移键控（改变频率）
- 相移键控（改变相位）
- 正交振幅调制（将振幅和相位的变化结合起来）

<em>模拟到模拟转换</em>
如果介质具有带通性或者只有带通宽带可以用，那么模拟信号需要进行调制。
调幅、调频、调相
...
允许同时通过一条数据链路传输多个信号
- 频分多路复用
- 波分多路复用。类似于频分多路复用，但是频率非常高，光纤电缆。
- 时分多路复用

<em>扩频</em>
扩频为无线应用而设计
扩频增加了冗余部分，扩大的带宽允许源端用有防护的封装将报文进行更安全的传输
- 跳频扩频。使用源信号调制M个不同的载波频率，不同时刻调制不同的频率
- 直接序列扩频。每个数据位用扩展编码的n位代替
...
<em>有损压缩</em>
- 预知编码，在脉冲编码调制中使用delta调制
- 变换编码，输入信号应用数学转换产生输出信号，导致编码数位减少。转换可逆。
  - 离散余弦变换（DCT）
...
- 图像
  - JPEG
    每个像素为使用24比特位
    ![JPEG转换过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-1.png)
    ![JPEG量化矩阵](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-2.png)
    ![JPEG“之”字编码过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-3.png)
  - GIF
    每个像素为使用8比特位，大小为JPEG的1/3
- 视频
  - MPEG。在空间上，使用JPEG压缩每一帧；在实践上，去掉冗余帧。P-帧只包含前一帧中变化的部分
- 音频
  - 预知编码。例如MP3
  - 感知编码。将数据从时域转换为频域，然后在频域执行数据操作。例如忽略频率低于100Hz，能量低于20dB的声音。

### 8.3 因特网中的多媒体
- 流式存储音频/视频。文件被压缩并存储在服务器上，用户按需请求压缩的音频/视频文件
  - 使用web服务器。用户在文件播放之前需要等待整个文件完整下载
  - 使用带有元文件的web服务器。媒体播放器先获得完整下载的元文件，在播放过程中边播边下
  - 使用媒体服务器。不同于Web服务器，媒体服务器使用UDP协议
  - 使用媒体服务器和实时流协议RTSP。使用RTSP可以控制音频、视频的播放
  - ![使用web服务器](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-4.png)    ![使用媒体服务器](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-5.png)    ![使用媒体服务器和RTSP](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-6.png)
- 流式实况音频/视频。
    类似于广播电台。对于延迟很敏感，不接受重传。实时流更适合于IP多播服务和使用UDP服务
    因特网广播、电视；IPTV
- 实时交互音频/视频。
    实时通信。
    <span>抖动</span>
    ...
    <span>转换</span>
    <span>混合</span>
    <mark>前向纠错</mark>无法使用重传来处理损坏或丢失的分组，因为延迟无法接受。
     - 使用汉明距离的纠错。为了纠正t位差错，最小汉明距离应该为2t+1
     - 使用XOR的纠错。将一个分组分成N块，创建所有块的异或结果，并发送者N+1块。如果某个块损坏或丢失，可以在接收站被创建
     - 块交错。允许一些小块丢失。逐列发送而不再逐行发送数据，这样每个分组会丢失一部分数据，而不会整个分组都丢失。![交错](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-7.png)
     - 结合汉明距离和交错。分组中冗余可以纠正t位，然后将m行交错，逐列发送。自动纠错的位数可以增至m X t
     - 混合高分辨率和低分辨率的分组。为每一个分组创建一个低分辨率冗余，并且将冗余版本与下一个分组合并。

### 8.4 实时交互协议
需要对应用层和传输层使用新的协议
应用层：MPEG音频、mPEG视频等。。。
传输层
  - 要求发送端-接收端协商（编码方式、压缩方式等）
  - 创建分组流。UDP无法提供帧之间的关系，TCP提供字节关系而不是帧之间的关系
  - 源同步。如果使用多个源，视频和音频需要同步
  - 纠错控制。TCP的重传机制无法适用
  - 拥塞机制
  - 防止抖动
  - 发送端认证
...
<em>实时传输协议RTP</em>
RTP没有分发机制，必须与UDP一起使用，位于UDP和应用程序之间
RTP使用偶数号的<span>UDP端口</span>  
![RTP位置](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-8.png)
![RTP头部格式](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-9.png)

<em>实时传输控制协议RTCP</em>
分组创建了一个带外控制流，在发送端和接收端提供反馈信息。
RTCP使用奇数号的UDP端口。  
![RTP分组类型](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-10.png)

<em>会话初始化协议SIP(Session Initiation Protocol)</em>
应用层协议
建立、管理和终止多媒体会话
端口号5060
请求报文
  ...
  - ACK。呼叫方确认
  - OPTIONS。查询机器功能
  - CANCEL。取消初始化，但不能终止呼叫
  - REGISTER。被叫方不可用时建立连接
  - BYE。终止会话
响应报文
  - 消息性质响应，1XX
  - 成功响应，2XX
...
![SIP简单会话](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-11.png)
![跟踪被叫方](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-12.png)

<em>H.323</em>
允许公共电话网络上的电话与连接到因特网上的计算机进行通话

<em>流控制传输协议（SCTP）</em>
<mark>服务</mark>
  - 进程到进程通信
  - 多流。如果流中一个被堵塞，其他的流依然可以发送数据
  - 多接口。保证容错性
  - 全双工通信
  - 面向连接的服务
  - 可靠的服务
...
<mark>特性</mark>
  - 传输序列号TSN，为数据块编号
  - 流标示符SI，多个流需要使用流标示符，每个快头部都携带SI
  - 流序列号SSN，流中的每个数据块都定义了流序列号
  - 分组
      ![SCTP分组](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-13.png)
  - 确认号

<mark>关联</mark>
SCTP中的连接称为关联，因为可以有<span>多接口</span>
  - 关联建立。四次握手。  
  ![四次握手](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-14.png)
  - 数据传输。双向数据传输，支持捎带。
  	- 多接口数据传输</span>
  	- <span>多流传输</span>. 数据传输使用TSN处理，数据传递使用SI和SSN控制
  	- 分段
  - 关联终止。不允许半关闭。三次握手

<mark>流量控制、差错控制</mark>不想看
<mark>重传</mark>类似于TCP，超时重传和三次相同SACK丢失块重传

### 8.5 服务质量
<em>特征</em>
  - 可靠性
  - 延迟
  - 抖动。分组从源端到目的端发生不一样的延迟，这种延迟变动就是抖动。
  - 带宽
...
在路由器中基于服务等级需求处理分组
-  <span>FIFO队列</span>
  先来的先处理，如果队列满了，后续分组被丢弃。
  分组到达队列时有一定延迟，而离开队列又有不同的延迟。所以有抖动
  默认调度算法，唯一可以保证的是按顺序离开队列
- <span>优先级队列</span>
  每个优先级都有自己的丢咧，优先级越高的队列越先被处理
  优先队列可以提供比FIFO队列更好的服务质量。
  容易出现饿死的情况，低优先级永远无法被处理
- <span>加权公平队列</span>
  不同优先级分配不同的时间比，例如最高优先级获得1/2，次一级1/3，最低级1/6等

<mark>流量整型或监控</mark>
流量数量或者速率的控制
- <span>漏桶算法</span>
  应对突发性流量。突发性的数据块被保存在桶中并以平均速率发送。![漏桶算法示意](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-15.png)
- <span>令牌桶算法</span>
  漏桶算法不适合空闲主机。
  令牌桶算法允许空闲主机以令牌的形式计算未来的信誉
  令牌桶允许规定的最大速率的突发性流量。![令牌桶最大平均速率](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C8-16.png)
- <span>合并漏桶和令牌桶算法</span>
  漏桶算法在令牌桶算法之后应用，漏桶速率快于令牌桶

<mark>资源预留</mark>
称为综合服务的QoS模型

<mark>进入许可控制</mark>
使用路由器或交换机接受或拒绝基于称为流格式的预先定义参数的流量

<em>综合服务(IntServ)</em>
基于流的体系结构，为给定的数据流明确预留资源
- 首先根据分组需求的服务将其分类
- 模型根据刘特征调度前向分组
- 设备通过进入许可控制判断设备是否有能力处理，然后做出承诺。如果超过路由器的处理能力，拒绝其进入

服务类型：
- 确保服务类型。不容错，保证分组在某一传递时间内到达，定量服务
- 控制负载服务类型。容错型自适应实时多媒体应用设计，允许延迟。定性服务，存在丢失分组的危险性

<span>资源预留协议(RSVP)</span>
从综合服务中分离出来的独立协议
面向连接，基于多播通信，由接收端决定加入或者离开多播组。

但是路由器负担过重，需要保留信息

<em>区分服务(DiffServ)</em>
主要处理过程从网络中心移动到网络边缘，解决扩展性问题
基于类别的服务，不再基于流
...
### 9.2 简单网络管理协议SNMP
提供一组基本的操作来监控和维护因特网。
管理器通常是一台主机，控制和监视一组代理（通常为路由器）

SNMP还使用另外两个协议<span>管理信息结构（Structure of Management Information， SMI）和管理信息库（Management Information Base，MIB）</span>。

SNMP定义管理器与代理之间交换分组的格式，读取和改变SNMP分组中对象的状态值。
SMI定义对象命名、数据类型以及编码方法和取值的一般规则，并且说明了如何对对象和值进行编码
MBI创建了一个被管理的实体中所有对象命名和类型，以及它们之间的相互关系。
![网络管理的作用](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C9-1.png)
![管理概要](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C9-2.png)
<mark>SMI</mark>
- 给对象命名
- 定义可在对象中存储的数据类型
...
<mark>MIB</mark>
![MIB的分组](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C9-3.png)

<mark>SNMP</mark>
- 管理器检查代理的方法是发出请求得到能反映代理行为的信息
- 管理器可用重新设置在代理数据库中的某些值来强迫代理完成一个任务
- 代理管理过程的方法是向管理器发送异常情况的报警
![协议数据单元PDU](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C9-4.png)
![SNMP PDU格式](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C9-5.png)

管理器使用熟知端口162，代理使用熟知端口161

### 9.3 ASN.1
抽象语法表示法1，应用在计算机科学的多个不同领域的非常复杂的语言

-----
## 十、网络安全
### 10.1 介绍
目标：
  - 机密性。防范危害信息存储和传输的恶意行为
  - 完整性。保证改变由授权的实体通过授权的机器进行
  - 可用性。创建和存储的信息应该对授权实体可用


![攻击分类](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-1.png)

<mark>密码系统</mark>利用密钥对信息进行加密和解密
<mark>信息隐藏</mark>通过其他的东西将信息覆盖从而隐藏信息本身

### 10.2 机密性
<em>对称密钥密码</em>
加密和解密使用相同的密钥，并且密钥可以用于双向通信。
需要一个安全的通道传递密钥

<mark>传统对称密钥密码</mark>
  - 替换密码。将一个符号替换成另一个符号。
    - 单字母密码。明文中的一个字符总是对应密文中的同一个字符。对于穷尽密钥搜索的攻击非常脆弱
    - 多字母密码。明文中的一个字符与密文中的一个字符为一对多的关系。密文中的每个字符既依赖于相应的明文字符，也依赖于明文字符在消息中的位置。
  - 换位密码。改变符号的位置
  - 流密码和块密码
    - 流密码。每次加密和解密一个符号。我们有一个明文流、一个密文流、一个密钥流
    - 块密码。长度为m的一组铭文符号一起被加密


<mark>现代对称密钥密码</mark>
现代需要加密的信息不只包括文本，还有多媒体数据
我们需要面向比特的密码
  - 现代块密码。由排列单元、替换单元、异或操作、移位元素、交换元素、分裂元素和组合元素组成
    ![现代块密码组成](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-2.png)
    异或操作是重要组成部分，应用密钥将n比特的数据片与n比特的密钥进行混合
    数据加密标准（DES）。64位明文通过16轮次加密最终生成64位密文。
    ![DES的一般流程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-3.png)
  - 现代流密码。需要加密二进制流并且按照常数速率传输时，使用流密码。更好抵抗传输过程中的毁坏能力。
    最安全、简单的同步流加密称为一次一密，利用一个随机选择的密钥流进行每一次的加密。非常完美的密码，对手不能猜测密钥或统计特征。但实现非常困难

<em>非对称密钥密码</em>
秘密信息是私有的，每个人创建并持有他或她自己的秘密信息。
非对称密钥密码基于数学函数的应用。
非对称密钥密码使用两个分离的密钥：一个私钥一个公钥。
公钥加锁，私钥解锁

- 提供安全性的责任主要由接收方承担。接收方必须提供公钥发布，并保证其完整性
- 团体中每个实体都需要构建自己的私钥。
- 每个实体需要一个私钥接受其他人响应，并需要n个私钥来与团体中的n个实体进行通信

明文和密文被当做整数对待，在加密前消息必须编码为一个整数。
<span>非对称密钥密码系统通常用于加密或解密小块信息，比如一个对称密钥密码系统中的密钥密码</span>

<mark>RSA密码系统</mark>。利用大数。![RSA的加密、解密和密钥生成](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-4.png)

### 10.3 安全的其他方面
<em>消息的完整性</em>
消息应该保持未被修改状态
<mark>消息和消息摘要</mark>。使用指纹(fingerprint)。消息通过一个<span>加密散列函数</span>。构建一个称为摘要(digest)。再次运行加密散列函数，如果新摘要与原来的摘要一致那么消息没有被篡改。
![消息和摘要](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-5.png)
<span>MD5(信息摘要比较小以至于不能抵抗攻击)</span>
<span>安全散列算法(SHA)</span>

<em>消息认证</em>
完整性能够保证消息没有被修改，但不能确定消息来源。
需要创建一个消息认证码(MAC)，保证创建人身份。
![消息认证码](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-6.png)
但是注意，共享的秘密密钥K需要安全通道传递

<em>数字签名</em>
MAC利用秘密密钥保护摘要，而数字签名使用私钥-公钥对。
![数字签名处理过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-7.png)

如果A向B发送消息，随后又否认它，并修改了自己的数字签名。可以通过一个可信中心，将数字签名都归档和保存。

对于长消息，签名和验证可以采用信息摘要。也就是说，先进行一次摘要，再对摘要进行加密和验证。

数字签名标准（DDS）

<em>实体认证</em>
用来让一方验证另一方身份。
实体认证不同于消息认证，是实时的，并且在整个会话期间贯穿始终
  - 密码。申请者通过阐明知道的秘密来证明其身份。但是密码很容易被盗取、截获、猜测
  - 挑战-响应。申请者证明她知道一个秘密但不用发送它。验证者发送挑战（随时间变化的值），申请者对这个挑战应用一个函数并向验证者发送结果，成为响应。
    - 使用对称密钥密码
    - 使用非对称密钥密码
    - 使用数字签名
...
<em>密钥管理</em>
<mark>对称密钥发布</mark>
如果要和N个人通信，那么需要保存N(N-1)/2个密钥。无法保证这么多密钥在互联网上传播的安全。

- 密钥发布中心(KDC)。当使用KDC人越来越多，可以使用多重KDC。
  当A要和B通信时，A想KDC发送一个请求，KDC将请求发送给B,B同意后在AB之间创建一个会话密钥。
  会话密钥只使用一次。
  ![KDC创建会话密钥](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-8.png)
  第2步中，KDC认证了Alice，因为第二层加密是使用Alice的密钥加密的
  第3步中，KDC认证了Bob，因为第一层加密是使用Bob的密钥加密的
- 对称密钥协商。Diffie-hellamn密钥协商
  双方选择两个数p和g，不需要保密
  ![Diffie-Hellman方法](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-9.png)

<mark>公钥发布</mark>
- 公共通告。在网站或者报纸上发布。但是容易遭到假冒。
- 认证中心(CA)。CA使用自己的私钥对证书进行加密，任何人可以下载并用CA的公钥解密得到证书

### 10.4 Internet安全
<em>应用层安全</em>
两种对电子邮件提供安全服务的协议.
电子邮件安全中，加密/解密使用对称密钥算法。但是密钥使用接受者的公钥进行加密并随邮件一起发送。

<mark>良好隐私协议（PGP, pretty Good Privacy）</mark>
对电子邮件提供保密性和完整性保护，并对电子邮件进行认证。
- 对邮件进行压缩
- 对压缩后的消息进行摘要，并用发送者私钥进行签名
- 对压缩后的消息和摘要使用一次性密钥进行加密
- 对使用的一次性密钥使用接受者的公钥加密，并随邮件一起发送
- 接受者使用私钥解密一次性密钥，从而获得邮件和加密的摘要
- 接受者使用发送者的公钥验证摘要的有效性
![PGP加密过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-10.png)

PGP中每个人都有一个私钥环和一个公钥环。
PGP证书不存在CA，任何人都能为其他人签署证书。证书可以在网络中通过介绍人传递，使得信任网络越来越大。

<mark>S/MIME</mark>
安全多用途因特网邮件扩展，是MIME协议的扩展
<span>安全消息语法</span>
- Signed-Data: 提供数据的完整性。签署者利用自己选择的特定散列算法生成消息摘要，利用私钥对摘要加密，然后将内容、签名、证书和算法整合为signed-data对象。![Signed-Data内容](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-11.png)
- Enveloped-Data：提供消息的保密性。类似于PGP中的加密过程。![Enveloped-Data内容](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-12.png)
- Digested-Data：提供消息的完整性，其结果一般作为Enveloped-Data的内容。![Digested-Data内容](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-13.png)
-  Encrypted-Data：创建任意内容类型的加密版本，不同于enveloped-data，内容没有接受者；只用于加密信息而不用于传递
-   Authenticated-Data：用于提供数据的认证。（注：MAC为消息认证码）![Authenticated-Data内容](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-14.png)

<em>传输层安全</em>
提供服务器和客户的认证、数据机密性和完整性
<mark>SSL(Secure Socket Layer)协议</mark>
服务：
  - 分段
  - 压缩
  - 消息完整性。利用密钥散列算法构建MAC
  - 保密性。原始数据和MAC利用对称密码加密
  - 成帧

需要4个密钥和两个IV（初始化向量）。客户和服务器各需要一个密钥进行消息认证、一个密钥进行加密、一个IV作为初始化块。
一个方向上的密钥要不同于另一个方向，这样一个方向上的攻击不会影响另一个方向。

四个协议：  
![SSL四个协议](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-15.png)
  - 握手协议。利用消息来协商密码组，如需要，也实施客户端认证服务和服务端认证服务。分为4各阶段
    1. 通告安全能力，建立会话ID并选择密码组，协商压缩方法，各选择一个随机数用于生成主密码。
    2. 服务器认证自己，发送自己的证书和公钥
    3. 客户端认证自己。此时两者都知道了预主密码
    4. 发送消息以改变密码规格，完成握手协议


![SSL握手协议](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-16.png)
  - 改变密码规格协议。握手协议中已经进行密码协商并生成了密码。直到发送和收到改变密码规格这个特殊消息，才能开始使用这些密码
  - 警告协议。报告错误和非正常情况。使用一个消息描述问题和问题级别。
  - 记录协议。承载来自于上层（其他三个协议）的消息，消息被分段、压缩、构建MAC、加密、添加SSL头部等。![记录协议过程](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-17.png)

<mark>TLS(Transport Layer Security)协议</mark>
前者的 IETF版本

<em>网络层安全</em>
IP层安全（IPSec）帮助生成认证和加密
<mark>两种模式</mark>
- 传输模式
  传输模式不保护IP头部，只保护从传输层传下来的数据
  一般用于主机到主机（端到端）的通信中    
  ![传输模式](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-18.png)
- 隧道模式
  隧道模式保护整个IP分组，处理包括头部在内的IP分组，并添加一个新的头部
  一般用于路由器之间、主机到路由器、路由器到主机的通信中    
  ![隧道模式](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-19.png)

<mark>两种安全协议</mark>
- 认证报头协议(AH)
  用于认证源主机，并保证IP分组有效载荷的完整性。但是不保证机密性    
  ![AH协议](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-20.png)
- 封装安全载荷协议（Encapsulating Security Payload，ESP）    
  ![ESP协议](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-21.png)
  ESP能够完全取代AH，所以现在不需要AH了    
  ![IPSec服务](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-22.png)

<mark>安全关联（SA）</mark>
IPSec的重要特征。IPSec要求主机之间建立安全关联的逻辑关系。

安全关联就是双方之间的一个协议，创建双方之间的一个通道
SA分为出SA和入SA
...
<mark>安全策略（SP）</mark>
定义了分组发送或到达时的采用的策略。使用SA之前必须确定SP
安全策略数据库
出SPD，入SPD    
![出SPD处理](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-23.png)
![入SPD处理](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-24.png)

<mark>因特网密钥交换(IKE)</mark>
用于创建入和出安全关联
当一个分组要发送时，需要通过安全策略数据库查看是否存在这种流量类型的SA。如果没有SA，使用IKE创建一个

<mark>虚拟专用网（VPN）</mark>
使用Internet，使用公用物理网络但实际上是专用的
利用ESP封装私有数据报。Internet负责将数据从R1传到R2，但局外人不能解码分组内容或地址。解码发生在R2。  
![VPN](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-25.png)

### 10.5 防火墙
防火墙用于转发一些分组并过滤掉一些分组
- 分组过滤防火墙
  基于网络层或传输层的头部信息转发或阻止分组。包括源IP、目的IP、源端口、目的端口、协议类型。
  利用过滤表
  分组过滤防火墙在网络层或传输层进行过滤    
  ![分组过滤防火墙](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-26.png)
- 基于代理的防火墙
  基于代理的防火墙在应用层进行过滤
  安装一个代理计算机（应用网关，application gateway），位于客户计算机和服务器之间。当客户发送请求，应用网关在应用层打开分组并查看该分组是否合法。如果合法，扮演客户进程向服务器发送请求。如果不合法，丢弃该分组并向客户计算机发送错误信息    
  ![基于代理的防火墙](https://raw.githubusercontent.com/liujiajun1993/notes/master/images/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C10-27.png)
